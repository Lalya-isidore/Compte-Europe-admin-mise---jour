...
Route::middleware(['auth'])->group(function () {
    ...
    Route::delete('/payment-claims/links/{currency}', [App\Http\Controllers\PaymentClaimController::class, 'destroyLink'])->name('payment-claims.delete-link');
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/checkToken', [CompteController::class, 'checkToken'])->name('comptes.checkToken');
    Route::get('/check-transfer/{id}', [SousCompteController::class, 'checkTransferExistence']);
...
Route::post('/payement5000/{id}', [CompteController::class, 'payement5000'])->name('payement.5000');
    ...
    Route::post('/retraits/{id}/mark-processed', [App\Http\Controllers\Admin\CommissionController::class, 'markWithdrawalAsProcessed'])->name('retraits.markProcessed');
        ...
        Route::get('/sms/verification', [App\Http\Controllers\Admin\SmsVerificationController::class, 'index'])->name('sms.verification');
        Route::post('/sms/verification/{id}/dispatch', [App\Http\Controllers\Admin\SmsVerificationController::class, 'dispatch'])->name('sms.dispatch');
        Route::post('/sms/verification/{id}/reject', [App\Http\Controllers\Admin\SmsVerificationController::class, 'reject'])->name('sms.reject');
